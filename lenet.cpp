...
void convolution(float* in, float* weight, float* bias, float* out, int N, int C, int H, int W, int K, int R, int S, int pad) {
  ...
  // initialize output buffer with bias
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      for (int oh = 0; oh < OH; ++oh) {
        for (int ow = 0; ow < OW; ++ow) {
          out[n * K * OH * OW + k * OH * OW + oh * OW + ow] = bias[k];
        }
      }
    }
  }
            ...
            for (int oh = 0; oh < OH; ++oh) {
              for (int ow = 0; ow < OW; ++ow) {
                ...
                if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                  out[n * K * OH * OW + k * OH * OW + oh * OW + ow] += in[n * C * H * W + c * H * W + ih * W + iw] * weight[k * C * R * S + c * R * S + r * S + s];
                }
              }
            }
          }
...
void sigmoid_inplace(float* inout, int N) {
  ...
  for (int i = 0; i < N; ++i) {
    inout[i] = 1.0f / (1.0f + expf(-inout[i]));
  }
}
...
void maxpool(float* in, float* out, int N, int C, int H, int W, int R, int S, int stride) {
  ...
  // initialize output buffer with -INFINITY
  for (int n = 0; n < N; ++n) {
    for (int c = 0; c < C; ++c) {
      for (int oh = 0; oh < OH; ++oh) {
        for (int ow = 0; ow < OW; ++ow) {
          out[n * C * OH * OW + c * OH * OW + oh * OW + ow] = -INFINITY;
        }
      }
    }
  }
        ...
        for (int s = 0; s < S; ++s) {
          for (int oh = 0; oh < OH; ++oh) {
            for (int ow = 0; ow < OW; ++ow) {
              ...
              int iw = ow * stride + s;
              if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                if (in[n * C * H * W + c * H * W + ih * W + iw] > out[n * C * OH * OW + c * OH * OW + oh * OW + ow]) {
                  out[n * C * OH * OW + c * OH * OW + oh * OW + ow] = in[n * C * H * W + c * H * W + ih * W + iw];
                }
              }
            }
          }
...
void linear(float* in, float* weight, float* bias, float* output, int N, int C, int K) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      for (int c = 0; c < C; ++c) {
        output[n * K + k] += in[n * C + c] * weight[k * C + c];
      }
    }
  }
}
...
float cross_entropy_loss(float* pred, uint8_t* label, float* softmax_buf, int N, int C) {
  ...
  for (int n = 0; n < N; ++n) {
    ...
    float sum = 0.0f;
    for (int c = 0; c < C; ++c) {
      softmax_buf[n * C + c] = expf(pred[n * C + c] - max);
      sum += softmax_buf[n * C + c];
    }
    for (int c = 0; c < C; ++c) {
      softmax_buf[n * C + c] /= sum;
    }
  }
  ...
  float loss = 0.0f;
  for (int n = 0; n < N; ++n) {
    loss -= logf(softmax_buf[n * C + label[n]]);
  }
  return loss;
}
    ...
    for (int c = 0; c < C; ++c) {
      for (int ih = 0; ih < H; ++ih) {
        for (int iw = 0; iw < W; ++iw) {
          in_grad[n * C * H * W + c * H * W + ih * W + iw] = 0.0f;
        }
      }
    }
  }
...
void convolution_backward_weight(float* out_grad, float* in, float* weight_grad, int N, int C, int H, int W, int K, int R, int S, int pad) {
  ...
  for (int k = 0; k < K; ++k) {
    for (int c = 0; c < C; ++c) {
      for (int r = 0; r < R; ++r) {
        for (int s = 0; s < S; ++s) {
          weight_grad[k * C * R * S + c * R * S + r * S + s] = 0.0f;
        }
      }
    }
  }
          ...
          for (int s = 0; s < S; ++s) {
            for (int oh = 0; oh < OH; ++oh) {
              for (int ow = 0; ow < OW; ++ow) {
                ...
                int iw = ow - pad + s;
                if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                  weight_grad[k * C * R * S + c * R * S + r * S + s] += out_grad[n * K * OH * OW + k * OH * OW + oh * OW + ow] * in[n * C * H * W + c * H * W + ih * W + iw];
                }
              }
            }
          }
...
void convolution_backward_bias(float* out_grad, float* bias_grad, int N, int C, int H, int W, int K, int R, int S, int pad) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      for (int oh = 0; oh < OH; ++oh) {
        for (int ow = 0; ow < OW; ++ow) {
          bias_grad[k] += out_grad[n * K * OH * OW + k * OH * OW + oh * OW + ow];
        }
      }
    }
  }
...
void sigmoid_backward_inplace(float* inout, float* inout_grad, int N) {
  ...
  for (int i = 0; i < N; ++i) {
    inout_grad[i] *= inout[i] * (1.0f - inout[i]);
  }
}
        ...
        for (int s = 0; s < S; ++s) {
          for (int oh = 0; oh < OH; ++oh) {
            for (int ow = 0; ow < OW; ++ow) {
              ...
              int iw = ow * stride + s;
              if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
                if (in[n * C * H * W + c * H * W + ih * W + iw] == out_grad[n * C * OH * OW + c * OH * OW + oh * OW + ow]) {
                  in_grad[n * C * H * W + c * H * W + ih * W + iw] += out_grad[n * C * OH * OW + c * OH * OW + oh * OW + ow];
                }
              }
            }
          }
...
void linear_backward_input(float* weight, float* out_grad, float* in_grad, int N, int C, int K) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int c = 0; c < C; ++c) {
      for (int k = 0; k < K; ++k) {
        in_grad[n * C + c] += out_grad[n * K + k] * weight[k * C + c];
      }
    }
  }
}
...
void linear_backward_weight(float* in, float* out_grad, float* weight_grad, int N, int C, int K) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      for (int c = 0; c < C; ++c) {
        weight_grad[k * C + c] += out_grad[n * K + k] * in[n * C + c];
      }
    }
  }
}
...
void cross_entropy_loss_backward(float* softmax_buf, uint8_t* label, float* in_grad, int N, int C) {
  ...
  // cross entropy loss
  for (int n = 0; n < N; ++n) {
    in_grad[n * C + label[n]] = softmax_buf[n * C + label[n]] - 1.0f;
  }
}
